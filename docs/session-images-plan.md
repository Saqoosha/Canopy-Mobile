...
来るので、1 行に絵を付けるには結果を待つしかない。代償は、行が Read の完了時に
出ること、そして結果が来ないまま死んだ Read は上限に達するまで行が出ないこと。
**上限で追い出された分には素のレンチ行が出る**（ファイル名は出さず、件数だけ
ログに残る）—— pending から消えたエントリの `tool_result` はもう相手を見つけ
られないので、そこで行を出さないとその Read があったことごと沈黙する。

`firstImageResult` は 1 フレームから **1 枚しか**返さないので、複数の画像 Read の
結果が同じフレームに載ると 1 件を除いて画像の付かない行になる。理由と経緯は
