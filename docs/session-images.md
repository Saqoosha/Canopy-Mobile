...
- **結果が来ないまま死んだ Read は、上限に達するまで行が出ない。** `ImagePreviewScript` が「失敗した Read は装飾されないまま残る」を既に受け入れているのと同じ性質。上限で追い出された分には素の行が出る（下記）
...
**結果が来た Read は、必ず 1 行になる。** 最初はここに 2 つ穴があった。

- 失敗した Read（存在しないファイル、読めない画像）は `content` が配列ではなく文字列で来る。配列前提の抽出はこの形を `tool_result` として認識できず、pending エントリは解決されないまま残った —— 素のレンチ行どころか行そのものが出ない、「必ず行は出る」への直接の違反
- `firstImageResult` は 1 フレームから **1 枚しか**返さないので、複数の画像 Read の結果が同じフレームに載ると、1 件を除いて解決されないまま残った

どちらも `imageResults(inFrame:)` で塞いだ —— 1 フレームが解決するすべての `tool_result` の id を、見つかった画像とペアにして返す。呼び出し側はそれを全部回すので、そのフレームが解決するエントリは全部行になる。画像が読めて（かつアップロードも成功して）いればサムネイル付き、そうでなければ素のレンチ行。**「その Read が完了した以上、行は必ず 1 本出る」がここで初めて全経路で成立する。**
...
## バイトの居場所 —— R2 に 2 枚、DO には寸法だけ

イベント行が運ぶのは **寸法とバイト数だけ**（約 100 バイト）。鍵は運ばない —— イベント行の他のフィールドから導ける（下記）。full と thumb は R2。
...
保持は R2 のライフサイクルで **7 日**。リングバッファ 200 件は忙しい Mac なら数時間ぶんなので 7 日は十分に長い。**期限切れと、一度もアップロードされなかったものを、電話は区別しない** —— `GET /image` はどちらも同じ 404 を返し、電話に伝えられる違いが無いため。タップしたときの表示はどちらも共通で「Image unavailable」。
...
`SessionEventRecord` に optional なフィールドを足す（幅、高さ、バイト数）。鍵はここには無く、machine・sessionId・eventId から電話が組み立てる。無い relay からのイベントでも落ちないこと。
...
**電話側で書いていないもの**: サムネイルの描画、タップでの拡大、`NSCache` への出し入れ —— この機能の他の場所と同じ理由で、SwiftUI のビューテストやスナップショットは見送った。ここが壊れれば実機かシミュレータで見た瞬間に分かる類のもので、固定する価値が薄いと判断したため。
